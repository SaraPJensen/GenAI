...
model.save('saved_models/dgan_scaling.pt')
...
for idx, sample in enumerate(synthetic_scaled): 
    ...
    filepath = f'synthetic_data/dgan/dgan_scaling/dgan_synthetic_sample_{idx:03d}.csv'
    df.to_csv(filepath, index=False)


# Stack list of arrays into a single NumPy array
synthetic_scaled_array = np.stack(synthetic_scaled, axis=0)  # shape (num_samples, seq_len, num_features)
synthetic_scaled_reshaped = synthetic_scaled_array.reshape(-1, num_features)

print(np.mean(synthetic_scaled_reshaped))
print(np.max(synthetic_scaled_reshaped))
print(np.min(synthetic_scaled_reshaped))
