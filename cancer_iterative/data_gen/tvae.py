...
def make_tvae_data(percentage):
    ...
    progress.to_csv(f'loss_progress/{percentage}_train/tvae_{percentage}_cancer_loss.csv', index = False)

    # Save model
    synthesizer.save(f'saved_models/{percentage}_train/tvae_{percentage}_cancer.pkl')

    # Generate synthetic data
    synthetic_data = synthesizer.sample(num_rows=1000)

    synthetic_data.to_csv(f'../datasets/{percentage}_train/tvae_{percentage}_cancer_data.csv', index = False)
