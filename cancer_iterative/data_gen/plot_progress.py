...
def plot_all_gan_loss(modeltype, gan_part):
    ...
    for p, c in zip(range(10, 109, 10), colours):
        filepath = f'loss_progress/{p}_train/{modeltype}_{p}_cancer_loss.csv'
    ...
    fig.update_layout(
            title=dict(text=f'{gan_part} loss progress for {model_name}, cancer data', font=dict(size=28)),
    ...
    fig.write_image(f'loss_progress/{modeltype}_{gan_part}_progress_plot.pdf')
...
def plot_all_tvae_loss():
    ...
    fig.update_layout(
            title=dict(text=f'Loss progress for {model_name}, cancer data', font=dict(size=28)),
            xaxis=dict(title=dict(text = 'Epoch',
                                font = dict(size=20)), 
                        tickfont=dict(size=20)),
            yaxis=dict(title=dict(text = 'Loss',
...
def plotting_gan_loss(modeltype, percentage):
    ...
    filepath = f'loss_progress/{percentage}_train/{modeltype}_{percentage}_cancer_loss.csv'
    ...
    fig.update_layout(
        title=dict(text=f'Loss progress for {model_name}, {percentage} % of cancer data', font=dict(size=28)),
        xaxis=dict(title=dict(text = 'Epoch',
                            font = dict(size=18)), 
    ...
    fig.write_image(f'loss_progress/{modeltype}_{percentage}_progress_plot.pdf')
